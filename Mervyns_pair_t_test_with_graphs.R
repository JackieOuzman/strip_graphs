...
name_of_path <-
  file.path("W:", "value_soil_testing_prj", "Yield_data", "Tim_McClelland", "Mervyns",  "Mervyns_yld_2019_seg_ID.csv")
seg_ID <-read_csv(name_of_path)

graph_path <- file.path("W:", "value_soil_testing_prj", "Yield_data", "Tim_McClelland", "Mervyns")
...
### should get this from harms database
 ...
 str(harm_database)
 Tim_McClelland_Mervyns <- filter(harm_database,
                                   Paddock_tested == "Mervyns clay loam"
                                  #|Paddock_tested == "Top loam"  
                                  )%>% 
   dplyr::select(5: 11)


 Tim_McClelland_Mervyns
 
 
TSpecial <- ttheme_minimal(base_size = 8)
table1 <- tableGrob(Tim_McClelland_Mervyns , rows = NULL, theme=TSpecial )
