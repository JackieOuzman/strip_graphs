...
name_of_path <-
  file.path("W:", "value_soil_testing_prj", "Yield_data", "Jeff_Braun", "Bussenschutt",  "Buss_P_12Yld_Seg_ID_zone.csv")
seg_ID <-read_csv(name_of_path, col_types = cols(N_Rates = col_double(),  P_Rates = col_double()))

graph_path <- file.path("W:", "value_soil_testing_prj", "Yield_data", "Jeff_Braun", "Bussenschutt")
seg_ID
...
#Define the zones NO ZONES in strip
unique(seg_ID$zone)
#zone1 <- "Taylor Red"
#zone2 <- "Taylor Grey"
...
segments <- ggplot(seg_ID_rate1_2vsGR_summary, aes(SegmentID , YldMassDry, group = P_Rate_as_factor))+
  ...
  labs(x= "Distance along the strip",
       ...
       subtitle = "",
       caption = "")+
  #annotate("rect", xmin = 1, xmax = 11, ymin = 0, ymax = 4, alpha = .2) +
  #annotate("text", x = 5, y= 1,label = zone1)+
  
  #annotate("rect", xmin = 90, xmax = 99, ymin = 0, ymax = 4, alpha = .2)+
  #annotate("text", x = 95, y= 1,label = zone2)+
  annotate("text", x = 10, y= 1,label = "No soil sampling zone in main strip")
...
mean_zone_av_2 <-  group_by(zone_av_2, P_Rates) %>% 
  summarise(mean(YldMassDry))
positive_neg_value_GR_rate1 <- ifelse(filter(strip_av, P_Rates == rate1) - filter(strip_av, P_Rates == Grower_rate)>0, "plus", "minus") 
positive_neg_value_GR_rate1 <- positive_neg_value_GR_rate1[1,2]
positive_neg_value_rate2_GR <- ifelse(filter(strip_av, P_Rates == rate2) - filter(strip_av, P_Rates == Grower_rate)>0, "plus", "minus")
positive_neg_value_rate2_GR <- positive_neg_value_rate2_GR[1,2]


p_vlaue_text_strip <- paste0("Yield at P ", rate1, " is  P ", Grower_rate, " " ,positive_neg_value_GR_rate1, " ", 
                             strip_rate1vsGR_res_sig$rounded, " and is ", 
                             strip_rate1vsGR_res_sig$significant, "\n",
                              
                              "Yield at P ", rate2, " is  P ", Grower_rate , " " ,positive_neg_value_rate2_GR, " ", 
                             strip_rate2vsGR_res_sig$rounded, " and is ", 
 ...
 Pvalue_on_graph <- grobTree(textGrob(p_vlaue_text_strip, x=0.1,  y=0.10, hjust=0,
 ...
 ###############################################################################################################
 #Make an empty plot
 empty <- ggplot( strip_av, aes(P_Rate_as_factor, YLDMASSDR))+
   # +
   #   geom_boxplot(alpha=0.1)+
   #   geom_point(colour = "blue", alpha = 0.1)+
   theme_bw()+
...
print(p_vlaue_text_zone_1)
 ...
 zone_1
 ggsave(path= graph_path, filename = "t-test_zone_red_strip.png", device = "png" ,
        width = 20, height = 10, units = "cm")
 write.csv(zone_av_1, paste0(graph_path,"/t_testzone_red_av.csv"))
...
### should get this from harms database
 ...
 str(harm_database)
Jeff_Buss <- filter(harm_database,
                       Farmer == "Bussenschutt  1") %>% 
   dplyr::select(4, 6: 11)
...
#make a table of the mean yield for whole striop
mean_strip_av <- group_by(strip_av, P_Rates) %>% 
  summarise(mean(YldMassDry))
mean_strip_av
names(mean_strip_av) <- c("Rates", "Yield")
mean_strip_av <- round(mean_strip_av,2)
mean_strip_av <-  lapply(mean_strip_av, as.character)
mean_strip_av <- as.data.frame(mean_strip_av )
mean_strip_av

  TSpecial <- ttheme_minimal(base_size = 8)
table1 <- tableGrob(Jeff_Buss , rows = NULL, theme=TSpecial )
#table2 <- tableGrob(mean_zone_av_1_2, rows = NULL, theme=TSpecial)
table2 <- tableGrob(mean_strip_av, rows = NULL, theme=TSpecial)

#get the name of the paddock...

paddock <- "Bussenschutt"
...
#with NO Zones
collection <- grid.arrange(strip, empty,  table1, segments, table2,  nrow = 5, 
                           layout_matrix = cbind(c(1,1,5,4,4), c(2,2,3,4,4)))           
collection
