...
name_of_path <-
  file.path("W:", "value_soil_testing_prj", "Yield_data", "Tim_McClelland", "Backsheehans",  "Backsheehans_Yld_Seg_ID_zones.csv")
seg_ID <-read_csv(name_of_path)

graph_path <- file.path("W:", "value_soil_testing_prj", "Yield_data", "Tim_McClelland", "Backsheehans")
seg_ID
...
segments <- ggplot(seg_ID_rate1_2vsGR_summary, aes(SegmentID , YldMassDry, group = P_Rate_as_factor))+
  ...
  labs(x= "Distance along the strip",
       ...
       subtitle = "",
       caption = "")+
   annotate("rect", xmin = 18, xmax = 28, ymin = 0, ymax = 5, #Zone 1
           alpha = .2) +
  annotate("text", x = 23, y= 1,label = zone1)+
  
   annotate("rect", xmin = 75, xmax = 85, ymin = 0, ymax = 5, #zone 2
            alpha = .2)+
  annotate("text", x = 80, y= 1,label = zone2)
#+
...
zone_av_1_rate2vsGR <- filter(zone_av_1_rate2vsGR, SegmentID != 18 )
...
### should get this from harms database
 ...
 str(harm_database)
 Tim_McClelland_Backsheehans <- filter(harm_database,
                            Paddock_tested == "Backsheehans") %>% 
   dplyr::select(5, 6: 11)


 Tim_McClelland_Backsheehans
...
table1 <- tableGrob(Tim_McClelland_Backsheehans , rows = NULL, theme=TSpecial )
table2 <- tableGrob(mean_zone_av_1_2, rows = NULL, theme=TSpecial)

#get the name of the paddock...

paddock <- "Backsheehans"
