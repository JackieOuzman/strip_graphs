...
seg_ID_rate1_2vsGR_summary <- seg_ID_rate1_3vsGR_summary
...
segments <- ggplot(seg_ID_rate1_2vsGR_summary, aes(SegmentID , YldMassDry, group = P_Rate_as_factor))+
  ...
  labs(x= "Distance along the strip",
       ...
       subtitle = "",
       caption = "")+
   annotate("rect", xmin = 82, xmax = 92, ymin = 0, ymax = 6, #Zone 1
           alpha = .2) +
  annotate("text", x = 87, y= 1,label = zone1)+
  
   annotate("rect", xmin = 10, xmax = 20, ymin = 0, ymax = 6, #zone 2
            alpha = .2)+
  annotate("text", x = 15, y= 1,label = zone2)+
...
### should get this from harms database
 ...
 str(harm_database)
Craig_Muir_Arhurs <- filter(harm_database,
...
table1 <- tableGrob(Craig_Muir_Arhurs , rows = NULL, theme=TSpecial )
