...
##Bring in my data...
"\\FSSA2-ADL\clw-share1\Microlab\value_soil_testing_prj\Yield_data\Jeff_Braun\Clarke\Clarke_Yld_Seg_ID_zones.csv"


name_of_path <-
  file.path("W:", "value_soil_testing_prj", "Yield_data", "Jeff_Braun", "Clarke",  "Clarke_Yld_Seg_ID_zones.csv")
seg_ID <-read_csv(name_of_path, col_types = cols( P_Rates = col_double()))

graph_path <- file.path("W:", "value_soil_testing_prj", "Yield_data", "Jeff_Braun", "Clarke")
seg_ID
...
segments <- ggplot(seg_ID_rate1_2vsGR_summary, aes(SegmentID , YldMassDry, group = P_Rate_as_factor))+
  ...
  labs(x= "Distance along the strip",
       ...
       subtitle = "",
       caption = "")+
   annotate("rect", xmin = 1, xmax = 10, ymin = 0, ymax = 4, #Zone 1
           alpha = .2) +
  annotate("text", x = 5, y= 1,label = zone1)+
  
   annotate("rect", xmin = 21, xmax = 26, ymin = 0, ymax = 4, #zone 2
            alpha = .2)+
  annotate("text", x = 23.5, y= 1,label = zone2)#+
...
# positive_negative_rate1_GRS <- 
 ...
 p_vlaue_text_zone_2 <- paste0("Yield at P ", Grower_rate, " is  P ", rate1 , " " ,positive_neg_value_GR_rate1, " ", 
...
### should get this from harms database
 ...
 str(harm_database)
Jeff_Clarke <- filter(harm_database,
                       Farmer == "Tony Clark") %>% 
   dplyr::select(4, 6: 11)
...
table1 <- tableGrob(Jeff_Clarke , rows = NULL, theme=TSpecial )
