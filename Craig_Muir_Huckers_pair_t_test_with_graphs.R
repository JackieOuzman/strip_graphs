...
segments <- ggplot(seg_ID_rate1_2vsGR_summary, aes(SegmentID , YldMassDry, group = P_Rate_as_factor))+
  ...
  labs(x= "Distance along the strip",
       ...
       subtitle = "",
       caption = "")+
   annotate("rect", xmin = 30, xmax = 39, ymin = 0, ymax = 4, #Zone 1
           alpha = .2) +
  annotate("text", x = 34.5, y= 1,label = zone1)+
  
   annotate("rect", xmin = 2, xmax = 11, ymin = 0, ymax = 4, #zone 2
            alpha = .2)+
  annotate("text", x = 6.5, y= 1,label = zone2)
#+
