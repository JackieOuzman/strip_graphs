...
segments <- ggplot(seg_ID_rate1_2vsGR_summary, aes(SegmentID , YldMassDry, group = P_Rate_as_factor))+
  ...
  labs(x= "Distance along the strip",
       ...
       subtitle = "",
       caption = "")+
   annotate("rect", xmin = 49, xmax = 59, ymin = 0, ymax = 4, #Zone 1
           alpha = .2) +
  annotate("text", x = 54, y= 1,label = zone1)+
  
   annotate("rect", xmin = 5, xmax = 24, ymin = 0, ymax = 4, #zone 2
            alpha = .2)+
  annotate("text", x = 14.5, y= 1,label = zone2)
#+
...
p_vlaue_text_strip <- paste0("Yield at P ", rate1, " is  P ", Grower_rate,  " plus ", strip_rate1vsGR_res_sig$rounded, " and is ", 
 ...
 zone_1_all <- filter(seg_ID, zone == zone1 )
 zone_av_1_all <- group_by(zone_1_all,SegmentID, P_Rates ) %>% 
   summarise_all(mean)
 
 zone_av_1_all
 unique(zone_1_all$zone)
 
 group_by(zone_av_1_all, P_Rates) %>% 
  summarise(mean(YldMassDry))
 
zone_av_1_all$P_Rate_as_factor <- as.factor(zone_av_1_all$P_Rates)
...
#   filter(between(SegmentID, 10,17)) # rate 1
# zone_north_av_25vs50 <- zone_north_av_25vs50 %>% 
#   filter(between(SegmentID, 7,17)) # rate 2
# zone_north_av_100vs50 <- zone_north_av_100vs50 %>% 
#   filter(between(SegmentID, 13,17)) # rate 3
# 
...
zone_av_1_all 
zone_av_1_rate1vsGR <- filter(zone_av_1_all, P_Rates == rate1 | P_Rates== Grower_rate )
zone_av_1_rate2vsGR <- filter(zone_av_1_all, P_Rates == rate2 | P_Rates== Grower_rate )
zone_av_1_rate3vsGR <- filter(zone_av_1_all, P_Rates == rate3 | P_Rates== Grower_rate )

head(zone_av_1_rate1vsGR)

zone_av_1_rate2vsGR <- zone_av_1_rate2vsGR %>% 
      filter(between(SegmentID, 50,59)) # rate 2

zone_1_revised <- rbind(zone_av_1_rate1vsGR, zone_av_1_rate2vsGR, zone_av_1_rate3vsGR)
...
mean_zone_av_1 <-  group_by(zone_1_revised, P_Rates) %>% 
  summarise(mean(YldMassDry))
mean_zone_av_1
...
positive_neg_value_rate2_GR_zone1 <- ifelse(filter(mean_zone_av_1, P_Rates == Grower_rate) 
                                            - filter(mean_zone_av_1, P_Rates == rate2)>0, "plus", "minus")
...
print(p_vlaue_text_zone_1)
 ...
 zone_1_revised
 zone_1 <- ggplot( zone_1_revised, aes(P_Rate_as_factor, YldMassDry))+
   geom_boxplot(alpha=0.1)+
   geom_point(colour = "blue", alpha = 0.1)+
   stat_summary(fun.y = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..),
                width = .75, linetype = "dashed")+
 ...
 zone_av_2_rate1vsGR
  ...
  zone_av_2_rate1vsGR <- zone_av_2_rate1vsGR %>% 
       filter(between(SegmentID, 10,17)) # rate 1
  
  zone_av_2_rate2vsGR <- zone_av_2_rate2vsGR %>% 
     filter(between(SegmentID, 7,17)) # rate 2
  
  zone_av_2_rate3vsGR <- zone_av_2_rate3vsGR %>% 
     filter(between(SegmentID, 13,17)) # rate 3
  
 zone_2_revised <- rbind(zone_av_2_rate1vsGR, zone_av_2_rate2vsGR, zone_av_2_rate3vsGR)
  
  zone_av_2 <- group_by(zone_2_revised,SegmentID, P_Rates ) %>% 
    summarise_all(mean)
...
# positive_negative_rate1_GRS <- 
 ...
 positive_neg_value_rate2_GR <- ifelse(filter(mean_zone_av_2, P_Rates == Grower_rate) - filter(mean_zone_av_2, P_Rates == rate2)>0, "plus", "minus")
